...
export default {
  ...
  mcp: {
    tools: [
      {
        name: 'check',
        description: 'Force re-check all watched repos for workflow status changes right now',
        inputSchema: {
        ...
        async handler(
          ...
          const configRepos = (config.repos as string[]) ?? [];
          const discoveredRepos = prevState?.discoveredRepos as string[] | undefined;
          const targetRepo = params.repo as string | undefined;
          const checkRepos = targetRepo
            ? [targetRepo]
            : getEffectiveRepos(configRepos, discoveredRepos);

          if (checkRepos.length === 0) {
            return {
              text: 'No repos configured or discovered. Set owner or repos in plugins.d/actions-watcher.json',
              state: prevState ?? {},
            };
          }
          ...
          const results = await Promise.all(
            checkRepos.map(async (repo) => {
              const runs = await ghRunList(repo, limit, signal);
              let latest = getLatestPerWorkflow(runs, workflowFilter, branchFilter);
              if (maxAgeDays > 0) latest = filterStaleRuns(latest, maxAgeDays);
              return formatSessionStart(repo, latest, autonomy);
            }),
          );
      ...
      {
        name: 'discover',
        description: 'Re-discover repos with active workflows for the configured owner. Shows what repos are being watched and why',
        inputSchema: {
          type: 'object' as const,
          properties: {
            owner: {
              type: 'string',
              description: 'GitHub owner to discover for (defaults to configured owner)',
            },
          },
        },
        async handler(
          ...
          const owner = (params.owner as string) || (config.owner as string) || '';
          if (!owner) {
            return {
              text: 'No owner specified or configured. Set owner in plugins.d/actions-watcher.json or pass it as a parameter.',
              state: prevState ?? { repos: {} },
            };
          }

          const maxAgeDays = (config.maxAgeDays as number) ?? 14;
          const discovered = await discoverRepos(owner, signal);
          const configRepos = (config.repos as string[]) ?? [];
          const effective = getEffectiveRepos(configRepos, discovered);

          const lines = [
            `Discovered ${discovered.length} repos with workflows for ${owner} (reporting runs from the last ${maxAgeDays} days):`,
            ...discovered.map(r => `  • ${r}`),
          ];

          if (configRepos.length > 0) {
            lines.push('', `Plus ${configRepos.length} explicitly configured repos:`);
            lines.push(...configRepos.map(r => `  + ${r}`));
          }

          lines.push('', `Total watched: ${effective.length} repos`);
